...
const TerrainForm: React.FC<TerrainFormProps> = ({
  ...
  const uploadPhotos = async (photos: any[], folder: string = 'terrain-photos'): Promise<string[]> => {
  ...
  const onSubmitHandler = async (data: any) => {
    ...
    try {
      ...
      if (isValidationMode) {
        ...
        const existingValidationPhotoUrls = photoValidationUrls.filter(url => !url.startsWith('file:'));
        ...
        if (initialData?.id_terrain) {
          ...
          if (onSubmitSuccess) {
            ...
            const fixedTerrain = {
              ...newTerrain,
              surface_validee: newTerrain.surface_validee === null ? undefined : newTerrain.surface_validee,
          ...
          <Button 
            ...
            title={
              isSubmitting || isUploading ? (
                ...
                isValidationMode ? 
                  (form.watch('validation_decision') === 'rejetter' ? "Rejeter le terrain" : "Valider le terrain") : 
