...
const PaymentTrackingSection: React.FC<PaymentTrackingSectionProps> = ({
  ...
  return (
    <View className="space-y-4">
      <Text className="text-lg font-semibold mb-2">Suivi des Paiements</Text>
      
      <View className="flex-row flex-wrap justify-between gap-3">
        {/* Total Invested Card */}
        <View className="bg-white p-4 rounded-lg shadow-sm flex-1 min-w-[48%]">
          <View className="flex-row justify-between items-center pb-2">
            <Text className="text-sm font-medium">Total Investi</Text>
            <MaterialIcons name="trending-up" size={16} color="#64748b" />
          </View>
          <Text className="text-xl font-bold">{formatCurrency(metrics?.totalInvested ?? 0)}</Text>
          <View className="mt-2">
            <ProgressBar
        ...
        <View className="bg-white p-4 rounded-lg shadow-sm flex-1 min-w-[48%]">
          ...
          <View className="mt-2 border border-gray-200 rounded-full px-2 py-1 self-start">
