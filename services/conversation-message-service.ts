...
export async function getUsers({ id }: { id: string }): Promise<Utilisateur> {
  try {
    const { data, error } = await supabase
      .from('utilisateur')
      .select('nom, prenoms, photo_profil, email')
    ...
    const utilisateur: Utilisateur = {
      ...
      photo_profil: data?.photo_profil || 'https://ui-avatars.com/api/?name=User',
      email: data?.email || '',
    }
